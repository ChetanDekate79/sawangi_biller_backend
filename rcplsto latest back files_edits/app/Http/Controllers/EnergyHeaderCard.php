...
class EnergyHeaderCard extends Controller
{
    ...
    function getHeaderDataLive(){
        ...
        $data = cache()->remember($cacheKey, Carbon::now()->addMinutes(1), function (){
			$latest_date_time = DB::select('select date , time(dt_time) as time from productivity_data_rs485_summary
    ...
    function getHeaderDataForDate($date){
        $header =  DB::select("
        select DATE,max(category_color) as category_color  from productivity_hour_summary T1 
